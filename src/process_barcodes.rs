...
pub mod process_barcodes {
    ...
    pub fn process_barcodes(bar_found: HashMap<String, i32>, min_count: i32, min_barcodes: usize) -> (String, String, String) {
        ...
        let mut mixture = "no";
        if vect_lineages.len() > 1 {mixture = "yes";}
        ...
        for (barcode_id, nb_occurences) in &b_found {
            ...
            if nb_occurences >= &min_occurences {
                ...
                match merged_lineages.get(&lineage) {
                    Some(_vect_nb) => {merged_lineages.get_mut(&lineage).unwrap().push(nb_occurences.to_owned());}
                    None => {
                        merged_lineages.insert(lineage.clone(), Vec::new());
                        merged_lineages.get_mut(&lineage).unwrap().push(nb_occurences.to_owned());
                    }
                }
            }
        }
    ...
    fn format_data(data: HashMap<String, Vec<i32>>) -> String {
        ...
        sorted_keys
            ...
            .map(|&key| {
                ...
                let values_string = values.iter().map(ToString::to_string).collect::<Vec<String>>().join(", ");
    ...
    fn filter_lineages(lineages: HashMap<String, Vec<i32>>, min_barcodes: usize) -> HashMap<String, i32> {
