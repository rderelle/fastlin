...
use analyse_sample_bam::scan_bam;
...
fn get_data_type(name_sample: String, vec_files: Vec<PathBuf>) -> String {
    ...
    for file_path in vec_files {
        if let Some(file_str) = file_path.to_str() {
            if file_str.ends_with(".fna.gz")
                || file_str.ends_with(".fas.gz")
    ...
    let mut result = "";
    if count_fasta == 1 && count_fastq == 0 && count_bam == 0 {
        result = "assembly";
    } else if count_fasta == 0 && count_bam == 0 && count_fastq == 1 {
        result = "single";
    } else if count_fasta == 0 && count_bam == 0 && count_fastq == 2 {
        result = "paired";
    } else if count_fasta == 0 && count_fastq == 0 && count_bam == 1 {
        result = "BAM";
    } else {
        eprintln!(
            "error: sample {} has {} fasta, {} fastq and {} BAM files",
            name_sample, count_fasta, count_fastq, count_bam
        );
        process::abort();
    }
    result.to_string()
...
fn main() {
    ...
    sorted_samples.par_iter().for_each(|(sample, list_files)| {
        ...
        if data_type == "assembly" {
            ...
            // analyse BAM file
            let (barcode_found, coverage, error_message) = scan_bam(
                list_files.to_vec(),
                barcodes.to_owned(),
                &args.kmer_size,
                genome_size,
